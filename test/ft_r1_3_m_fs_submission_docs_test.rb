...
require_relative '../lib/fair_tests/ft_r1_3_m_fs_submission_docs'

class FtR13MFsSubmissionDocsTest < Minitest::Test
  include ::TestHelper
  include ::FtR13MFsSubmissionDocs

  def test_passes_when_search_process_follows_conditions
    stub_fairsharing_record(record_with_metadata([
                                                   {
                                                     'name' => 'Export',
                                                     'type' => 'read/write',
                                                     'access_method' => 'User interface',
                                                     'documentation_url' => 'https://support.datacite.org/reference/introduction'
                                                   }
                                                 ]))
    ...
    assert_includes last_response.body, 'documents user-facing data submission.'
  end
  ...
  def record_with_metadata(processes)
    {
      'id' => '1234',
      'registry' => 'Database',
      'metadata' => {

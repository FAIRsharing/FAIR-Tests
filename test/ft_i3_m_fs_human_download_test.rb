...
require_relative '../lib/fair_tests/ft_i3_m_fs_human_download'

class FtI3MFsHumanDownloadTest < Minitest::Test
  include ::TestHelper
  include ::FtI3MFsHumanDownload

  def test_passes_when_search_process_uses_user_interface_for_read_human_download
    ...
    assert_includes last_response.body, 'the database allows content download by humans.'
  end

  def test_passes_when_browse_process_uses_user_interface_for_read_write_human_download
    stub_fairsharing_record(record_with_processes([
                                                   {
                                                     'name' => 'Export',
                                                     'type' => 'read/write',
  ...
  def test_fails_when_search_process_is_not_user_interface_human_download
    stub_fairsharing_record(record_with_processes([
                                                    {
  ...
  def test_fails_when_no_data_processes_are_present_human_download
    stub_fairsharing_record(record_with_processes([]))
  ...
  def test_fails_when_user_interface_process_is_not_readable_human_download
    stub_fairsharing_record(record_with_processes([
                                                   {
                                                     'name' => 'Export',
                                                     'type' => 'write',
  ...
  def test_fails_when_identifier_is_not_a_fairsharing_record_human_download

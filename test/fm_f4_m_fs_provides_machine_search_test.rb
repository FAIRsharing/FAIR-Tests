...
class FmF4MFsProvidesMachineSearchTest < Minitest::Test
  ...
  def test_passes_when_search_process_uses_machine_interface_for_read
    ...
    assert_includes last_response.body, 'A search or browse machine interface function was found'
  end

  def test_passes_when_browse_process_uses_machine_interface_for_read_write
    stub_fairsharing_record(record_with_processes([
                                                   {
  ...
  def test_fails_when_machine_interface_process_is_not_readable
    stub_fairsharing_record(record_with_processes([
                                                   {
                                                     'name' => 'Search records',
                                                     'type' => ['write'],

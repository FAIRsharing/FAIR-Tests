...
require_relative '../lib/fair_tests/ft_r1_2_m_fs_maintaining_organisation'
...
class FtR12MFsMaintainingOrganisationTest < Minitest::Test
  ...
  def test_fail_ft_r1_2_m_fs_maintaining_organisation
    stub_request(:post, "#{ENV['FAIRSHARING_API_URL']}").
      with(headers: headers).to_return(
      status: 200,
      body: {
  ...
  def test_indeterminate_ft_r1_2_m_fs_maintaining_organisation
    stub_request(:post, "#{ENV['FAIRSHARING_API_URL']}").
      with(headers: headers).to_return(
      status: 200,
      body: {

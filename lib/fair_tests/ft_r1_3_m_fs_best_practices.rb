...
module FtR13MFsBestPractices
  require_relative '../fair_test_utils'
  include FairTestUtils



  def ft_r1_3_m_fs_best_practices(url_record)
    record = get_fairsharing_record(url_record)



    meta = {
      testid: 'FT_R1_3_M_FS_BestPractices.ttl',
      testname: 'FAIR Test - R1.3 - Metadata - Resource follows community-relevant best practices',
      description: 'This test evaluates whether the database, standard or policy follows community-relevant best practices. To satisfy this test, at least one related reporting guideline must have a subject that is identical to or a child of at least one assigned subject within the resource record. In FAIRsharing, this corresponds to reporting guideline records related via any non-deprecates relationship and curated using the subject tags outlined above. This test expects as input the FAIRsharing DOI or URL for the FAIRsharing record under evaluation.',
      keywords: ['FAIR', 'R1.3', 'FAIRsharing', 'community-relevant best practices'],
      creator: 'https://orcid.org/0000-0001-9572-0972',
      indicators: [],
      metric: 'https://doi.org/10.25504/FAIRsharing.8c1739',
      license: 'https://creativecommons.org/licenses/by/4.0/',
      testversion: '1.0.0',
      protocol: 'https',
      host: 'ostrails.github.io',
      basePath: '/assessment-component-metadata-records/test/',
      endpoint_url: 'https://fair-tests.fairsharing.org/test/ft_r1_3_m_fs_best_practices',
      endpoint_description: 'https://fair-tests.fairsharing.org/test_descriptions/ft_r1_3_m_fs_best_practices/api',
    }
    ...
    if record && !record.empty?
      ...
      if %w[Database Policy Standard].include? record['registry']
        ...
        if pass
          response.score = 'pass'
          response.comments << 'Using FAIRsharing metadata for the record under evaluation, the resource follows community-relevant best practices.'
        else
          response.score = 'fail'
          response.comments << 'Using FAIRsharing metadata for the record under evaluation, the resource does not follow community-relevant best practices.'
        end
      else
        response.score = 'fail'

module FtF1MMetadataIdPersistent
  ...
  def ft_f1_m_metadata_id_persistent(url_record)
    ...
    meta = {
      ...
      description: "This metric evaluates whether the metadata retrieved from the provided URI contains an identifier that satisfies the FAIRsharing definition of guaranteed persistence, as aligned with the EOSC PID Policy. Note that it assesses the metadata retrieved from the URI rather than the URI itself. During assessment, the provided URI is resolved in accordance with FAIRsharing’s identifier resolution best practices. If the record contains DOI or ARK identifiers it will pass. If any identifier URLs can be determined as persistent via FAIRsharing data the test will also pass. Otherwise, it will fail. Expected input is the FAIRsharing DOI or URL for the FAIRsharing record under evaluation.",
